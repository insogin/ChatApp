...
public class ExpressionUtil {
	...
	public static void dealExpression(Context context,SpannableString spannableString, Pattern patten) 
		...
		while (matcher.find()) {
			...
			int resId = Integer.parseInt(field.get(null).toString()); // 通过上面匹配得到的String来生成图片资源id
			if (resId != 0) {
				Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), resId);
				ImageSpan imageSpan = new ImageSpan(bitmap); // 通过图片资源id来得到bitmap
				int end = matcher.start() + key.length(); // 计算该图片名字的Length，也就是要替换的String的长度
				spannableString.setSpan(imageSpan, matcher.start(), end, 
					Spannable.SPAN_INCLUSIVE_EXCLUSIVE); // 将该图片替换String中规定的位置中
			}
		}
	}
